#!/usr/bin/python 
# $Id: main.py 7 2010-01-13 12:16:47Z mark $
#
# main
#
...
# Different search functions defiend in mdtf, mdtf1, and alphabeta
# You pick the one you want by setting searchfunction to  <variant>.best
# By convention best returns a 5-tuple.  See alphabeta.py for details

#searchfunction = mdtf.best
searchfunction = alphabeta.best
...
def help():
    ...
    print "dump         dumps data structures for debugging"
...
def mybest():
    ...
    if len(mvlist)>0:
        ...
        if not silent:
            ...
            print "ttHits=%s   |WTT|=%s  |BTT|=%s" % (alphabeta.ttHits,len(alphabeta.WTT),len(alphabeta.BTT))
        move.make(mv)
        auto()
        if verbose:
            board.dump()
...
def mybest_static(silent=False):
    ...
    if len(mvlist)>0:
        ...
        if not silent:
            if move.color(mv)=='white':
                msg = "After %s %s [%s]" % (move.num(mv),san.pr(mv,mvlist),score)
            else:
                msg = "After %s ...%s [%s]" % (move.num(mv),san.pr(mv,mvlist),score)        
        move.make(mv)
        if not silent:
...
def hammer_best():
    ...
    try:
        while True:
            if not mybest() or kingsonly() or len(board.history)>200:
